...
Every year thousands of scientific articles are published concerning the chronic neurodegenerative disorder and leading cause of dementia, Alzheimer’s disease (AD). Systematic reviews for AD research are labour intensive, and limit the timely dissemination of information. *In silico* machine learning methods are unbiased and have previously been used to comprehensively describe the AD research landscape. Currently there are no treatments that cure AD, however the second anti-amyloid disease-modifying immunotherapy, lecanemab, was granted accelerated approval by the FDA on `r format(as.Date(leca_approv), "%d-%m-%Y")`. Using LDA topic modelling, we aimed to compare the research themes in AD in the years surrounding the accelerated approval of lecanemab using full abstract text from PubMed and preprint database. Despite the frequency of literature increasing concerning terms relating to lecanumab, topics mentioning neurodegeneration, study terminology, and cellular and molecular pathologies have remained consistent surrounding its approval. Increasing per-topic-per-word probabilities for 'tau' and 'placebo' may suggest the research is shifting towards new therapeutic targets and advances in later phase clinical trials, however a longer follow up period will be necessary to validate these findings.
...
![*Unigram Tokenisation*](data/preprocess1.png){#fig-preprocess1}

![*Removal of Stop Words*](data/preprocess2.png){#fig-preprocess2}
...
::: {#fig-tokenisation layout="[[100, 100], [60, 40]]"}
```{r}
#| label: fig-unigram 
#| fig-subcap:
#|  - "Unigram"
...
![*Pre-Lecenamab Accelerated Approval: Top 10 unigrams in each LDA topic*](plots/pre-leca-lda-updated-blue2.png){#fig-pre-leca-lda}

![*Post-Lecenamab Accelerated Approval: Top 10 unigrams in each LDA topic*](plots/post-leca-lda-updated-blue2.png){#fig-post-leca-lda}
