...
![](data/uniofyork.png){fig-align="center" height="2in"}
:::
...
lm_gamma <- lm(gamma ~ as.factor(topic) * type, data = abstract_gamm)
summary(lm_gamma)

# Applied the model the effects were significant because there was a number of data points, but the variation in gamma explained by the model is so small to be meaningless 
...
# $type
#                             diff           lwr          upr p adj
# pre-leca-post-leca -3.330669e-16 -0.0001409819 0.0001409819     1
```
...
Lecanemab received accelerated and traditional approval in 2023 [@Office_of_the_Commissioner2023-hu; @Office_of_the_Commissioner2023-ka], however, we did not find any variability in the overall frequency of literature published containing terms associated with the MeSH term '*Alzheimer's Disease*', with 3,468 abstracts published before and 3,276 abstracts published after the accelerated approval of lecanemab ([@fig-results-summary; @fig-abstract-date]). The frequency of publications containing the terms associated with the AD drug lecanemab exponentially increasing in 2023 ([@fig-leca-date]), however, the significant variation in the per-document-per-topic probabilities (γ) explained by the two-topic LDA model was considered too small to be meaningful due to the large number of documents ([@fig-gamma-lda]). This suggests that the increase in abstracts concerning lecanemab may not have introduced a considerable amount of new vocabulary, therefore not separating the AD literature into two distinct topics.
