...
trigrams_united <- trigrams_united %>%
  mutate(trigram = str_replace_all(trigram, 
                                   "\\b(?:mild cognitive impairment|mci|mild impairment mci)\\b", 
...
This study found `r abstract_n` papers that were published between `r min(tidy_abstracts_clean$date)` and `r max(tidy_abstracts_clean$date)` that contained the identified terms (@tbl-inclusion criteria). 

The distribution of publication dates for these articles is shown in @fig-publication-date. There were `r nrow(filter(naive_drug_results, naive_drug_results$title %in% abstracts$title))` papers published between `r min(format(as.Date(naive_drug_results$date_published, format = "%Y %b %d"),"%d-%m-%Y"), na.rm = T)` and `r max(format(as.Date(naive_drug_results$date_published, format = "%Y %b %d"),"%d-%m-%Y"), na.rm = T)` that contained the identified terms.
...
The top 10 unigrams in LDA for **pre-leca:** and **post-leca** are shown in @fig-topic-model-unigrams.
...
#| include: true 
#| fig-cap: "**Unigram LDA Topic Modelling.** Outputs from the model with ten topics. Figures show the top ten most commonly associated unigrams in each topic ordered by their per-topic-per-word probability, β. Topic titles were manually created and added." 
#| fig-subcap: 
#|   - "Pre-leca" 
#|   - "Post-leca" 
#| fig-width: 15 
...
knitr::include_graphics("plots/pre-leca-lda-updated.png")
knitr::include_graphics("plots/post-leca-lda-updated.png")
```
