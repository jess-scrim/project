...
To reassure us that the PubMed search query encapsulated all literature, *litsearchr* was used to expand the search terms [@Grames2019-as]. Citations from the PubMed results, using the previous search criteria in @tbl-inclusion-criteria, were read into R. The combined unique keyword and titles, as not all articles have keywords, for each result were collected. To ensure only the most relevant terms were searched, stop words, containing very frequent terms that provide no significant information, such as 'the', 'and' or 'a', were removed. The minimum frequency of words for keywords and title was then set to n = 50 and n = 75, respectively. A matrix of each word in each article was created and the potential search terms were ranked with *create_network* and *strength* [@Barrat2004-hm] from the *igraph* package [@Csardi2006-gb]. The change point method calculated the optimal cutoff positions based on the trend in sharp changes for *strength*.
